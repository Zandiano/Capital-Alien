...
namespace HSE{
    ...
    House Constructor(const string& name, CA_Colors color, uint8_t value, uint8_t price, uint8_t FLAGS, uint8_t tier, HSETYPE type){
        return {name: name, color: GetColor(color), value: value, price: price, FLAGS: FLAGS, tier: tier, type: type};
    }
    ...
    void SetHouseColor(House& house, CA_Colors color){
        house.color = GetColor(color);
    }
